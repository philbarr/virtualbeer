...
import com.simplyapped.libgdx.ext.vuforia.renderer.VuforiaRendererDecorator;
 ...
 * 
 * @author mzechner and pbarr*/
public class VuforiaAndroidGraphics implements Graphics, Renderer {
  final View view;
  int width;
  int height;
  VuforiaAndroidApplication app;
  GLCommon gl;
  GL10 gl10;
  GL11 gl11;
  GL20 gl20;
  ...
  public VuforiaAndroidGraphics (VuforiaAndroidApplication vuforiaAndroidApplication, AndroidApplicationConfiguration config,
    ResolutionStrategy resolutionStrategy) {
    this.config = config;
    view = createGLSurfaceView(vuforiaAndroidApplication, config.useGL20, resolutionStrategy);
    setPreserveContext(view);
    view.setFocusable(true);
    view.setFocusableInTouchMode(true);
    this.app = vuforiaAndroidApplication;
  }
  ...
  private View createGLSurfaceView (Activity activity, boolean useGL2, final ResolutionStrategy resolutionStrategy) {
    ...
    if (useGL2 && checkGL20()) {
      ...
      view.setRenderer(new VuforiaRendererDecorator(this));
      return view;
    } else {
  ...
  public void onDrawFrame (javax.microedition.khronos.opengles.GL10 gl) {
    ...
    if (lresume) {
      ((VuforiaAndroidApplication)app).audio.resume();
      Array<LifecycleListener> listeners = ((VuforiaAndroidApplication)app).lifecycleListeners;
    ...
    if (lpause) {
      ...
      app.listener.pause();
      ((VuforiaAndroidApplication)app).audio.pause();
    ...
    if (ldestroy) {
      ...
      ((VuforiaAndroidApplication)app).audio.dispose();
      ((VuforiaAndroidApplication)app).audio = null;
